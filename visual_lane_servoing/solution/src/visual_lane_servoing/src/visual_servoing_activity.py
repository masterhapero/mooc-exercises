...
def get_steer_matrix_left_lane_markings(shape):
    ...
    speed = 0.55
    turbofactor = 1.10
    bfactor = turbofactor * speed / 0.22
    lsblur = 7.5 
    lstart = 68
    lmax = 15.5
    # tfac = 1.0
    ...
    if speed < 0.25:
        ...
        if speed < 0.65:
            ...
            for x in range(xa-lxl,xa+lxl):
                tfactor = bfactor
                cur = base+mul*x
                res[0:7, cur:(cur+1)] = dir * bfactor * lxplus
    ...
    for x in range(lxstart+lwoff,halfw+bw+lwoff):
        ...
        if speed < 0.25:
            ...
            if speed < 0.65:
                ...
                res[0:5, cur:(cur+1)] = dir*0.95*tfactor
                res[30:h, cur:(cur+1)] = dir*1.00*tfactor
                res[50:h, cur:(cur+1)] = 0.0
            else:
                tfactor = tfactor * xfac2
                # if x > 230:
                #    res[0:h, cur:(cur+1)] = dir*1.25*tfactor 
                #if x > 240:
                #    res[0:h, cur:(cur+1)] = dir*1.25*tfactor 
                #if x > 290:
                #    res[0:h, cur:(cur+1)] = dir*8.8*tfactor 
                # res[0:5, cur:(cur+1)] = dir*2.0*tfactor
                #if x > 290:
                #    res[30:h, cur:(cur+1)] = dir*5.50*tfactor
                # res[50:h, cur:(cur+1)] = 0.0

    if speed > 0.29:
        # draw y-wise
        lrangestart = lxstart+lwoff
        lrangestop = halfw+bw+lwoff
        lrange = lrangestop-lrangestart
        if speed > 0.65:
            lrangestart = lxstart + lwoff + int(bw/3)
        ymin = 50
        yoff = 0
        yext = 0
        if speed > 0.65:
        ...
        for y in range(ymin,h):
            ...
            if speed > 0.65:
                yoff = -30+int((y-ymin)/2.0)
                ymax = lrangestop-yoff+yext
                if ymax > lrangestop:
                    ymax = lrangestop
                res[y:y+1, lrangestart-yoff:lrangestart-yoff+int(lrange/4)] = dir * val/2.0 * bfactor * xfac2
                res[y:y+1, lrangestart-yoff+int(lrange/4):ymax] = dir * val*2.0 * bfactor * xfac2
            else:
                res[y:y+1, lrangestart:lrangestop] = dir * val * bfactor * xfac2
    ...
    if speed > 0.65:
        ...
        xa = halfw - 50
        lrangestop = halfw+bw+lwoff
        res[0:5,  180:lrangestop] = dir * bfactor * 0.2 * xfac2
        res[0:5,  xa:lrangestop] = dir * bfactor * 1.5 * xfac2
        res[5:10,  xa+30:lrangestop+20] = dir * bfactor * 1.5 * xfac2
        # res[0:2,  lrangestart:lrangestop] = dir * bfactor * 3 * xfac2
        res[2:8,  290:lrangestop+40] = dir * bfactor * 1.7 * xfac2
        res[8:30, 290:lrangestop+20] = dir * bfactor * 1.5 * xfac2      
        res[30:40, 290:lrangestop+10] = dir * bfactor * 1.5 * xfac2      
        res[40:50, 290:lrangestop] = dir * bfactor * 1.5 * xfac2      
        #res[50:w, lrangestop-1:lrangestop+40] = dir * bfactor * 1.5 * xfac2      
        #res[50:w, lrangestart-20:lrangestart+40] = 0.0      
...
def get_steer_matrix_right_lane_markings(shape):
    ...
    for x in range(180,halfw):
        ...
        if speed < 0.49:
            ...
            res[sp_+55:h, cur:(cur+1)] = dir*0.650*tfactor
        else:
            res[sp_+30:h, cur:(cur+1)] = dir*0.400*tfactor
            res[sp_+35:h, cur:(cur+1)] = dir*0.450*tfactor
            res[sp_+45:h, cur:(cur+1)] = dir*0.530*tfactor
            res[sp_+55:h, cur:(cur+1)] = dir*0.650*tfactor

    #ystart = base + mul*180
    #yend = base + mul*150
    #for y in range (rspoff2+65,h-50):
    #    val = 0.1
    #    if y >= 65:
    #        val = 0.3
    #    if y >= 75:
    #        val = 0.8
    ##        xoff = int((y-rspoff2-65))-20
    #    xoff = 0
    #    res[y:y+1, ystart+xoff:yend+xoff] = dir * val * bfactor * mfactor   
    #    #res[sp_+65:h, cur:(cur+1)] = dir*0.300*tfactor    
    #    #res[sp_+75:h, cur:(cur+1)] = dir*0.800*tfactor    
...
def detect_lane_markings(image):
    ...
    mask_left[:,int(np.floor(width/5*3))+80:width + 1] = 0
